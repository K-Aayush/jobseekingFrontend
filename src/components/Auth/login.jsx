...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { data } = await axios.post("/base-url/user/login", { email, password }, {
    ...
    <div className="flex flex-col md:flex-row justify-center items-center min-h-screen bg-white">
      ...
      <div className="lg:w-[33%] md:w-[50%] w-full p-8">
        <form onSubmit={handleLogin}>
          ...
          <span className="block text-center mb-6 text-gray-500">Log in to your account.</span>
          ...
          <p className="block text-center text-md text-gray-500">Don't have an account? <Link to="/register" className="font-semibold text-gray-700">Signup</Link></p>
        </form>
      </div>

      {/* Right side image */}
      <div className="lg:w-[67%] md:w-[50%] md:flex hidden relative mr-4">
        <img src={manLaptop} className="border inset-0 w-full h-full object-cover rounded-xl" alt="Background" />
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <h3 className="lg:text-3xl text-xl text-indigo-700 font-bold mb-4">Welcome Back!</h3>
            <p className="lg:text-lg text-md text-gray-500 mb-4 font-semibold">Forget the old rules. You can have the best people.Access your account and continue your journey towards finding the perfect job. Your dream job awaits!</p>
          </div>
        </div>
      </div>
    </div>
